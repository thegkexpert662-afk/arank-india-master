...
class AdminManagementScreen extends StatelessWidget {
  ...
  Future<void> _create(BuildContext context) async {
    ...
    try {
      await MasterDataService.createAdmin(
        name: name.text,
        email: email.text,
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Admin and Institute created successfully.')),
        );
      }
    } catch (e) {
      if (context.mounted) {
    ...
    return _ManagementPage(
      ...
      child: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        ...
        builder: (context, snap) {
          ...
          if (snap.hasError) {
            return Center(child: Text('Unable to load admins: ${snap.error}'));
          }
          ...
          return ListView.separated(
            ...
            itemBuilder: (_, i) {
              ...
              return _Tile(
                ...
                subtitle:
                    '${d['adminId'] ?? d.id}  •  ${d['email'] ?? ''}\nInstitute: ${d['instituteId'] ?? d['appId'] ?? '—'}',
                active: active,
                onChanged: (v) => MasterDataService.setAdminActive(d.id, v),
              );
            },
          );
        },
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(subtitle, style: const TextStyle(color: Color(0xff5F7180))),
                          ],
                        ),
                      ),
